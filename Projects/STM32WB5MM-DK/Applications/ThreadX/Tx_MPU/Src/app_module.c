...
/**
  ...
  * @brief   Tx Module's application file
  ******************************************************************************
  * @attention
  *
...
UCHAR  default_module_pool_space[DEFAULT_BYTE_POOL_SIZE] __attribute__((aligned (32)));
...
void    default_module_start(ULONG id)
{
    ...
    tx_byte_pool_create(byte_pool_0, "module byte pool 0", default_module_pool_space, DEFAULT_BYTE_POOL_SIZE);
...
void thread_0_entry(ULONG thread_input)
{
  ...
  status = txm_module_object_pointer_get_extended(TXM_QUEUE_OBJECT, "Resident Queue", 14, (VOID **)&resident_queue);
