...
####  <b>Expected success behavior</b>
  ...
  - GREEN LED toggles every 500ms for 5 seconds each time user presses the User button.
  - GREEN LED is Off when system is in LowPower mode.

#### <b>Error behaviors</b>

On failure, RED LED is toggled every 200ms while GREEN LED is turned OFF.
...
4. The User button interrupt priority must be set to the highest possible value, otherwise, after exiting STOP 2 Mode, the processor will not execute the interrupt handler.
...
#### <b>ThreadX usage hints</b>
 ...
 - ThreadX offers the "tx_application_define()" function, which is automatically called by the tx_kernel_enter() API.
   It is highly recommended to use it to create all ThreadX applications related resources (threads, semaphores, memory pools...)  but it should not in any way contain a system API call (HAL or BSP).
...
RTOS, ThreadX, Threading, Semaphore, PWR, STOP 2 Mode, Interrupt, EXTI, Wakeup, LowPower
